...
def parser_opt():
    ...
    parser.add_argument('--map_out_path', type=str, default='output/OPIX/ZPGNet')
    parser.add_argument('--model_path', type=str, default='logs/OPIX')
...
def main(model_path, save_path):
    ...
    # --------------------------------------------------------------------------------------#
    classes_path = 'model_data/HiXray_classes.txt'
    # --------------------------------------------------------------------------------------#
    ...
    #   默认指向根目录下的VOC数据集
    # -------------------------------------------------------#
    VOCdevkit_path = '/root/autodl-tmp/yolo3-pytorch-master/VOCdevkit/HiXray'
    # -------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    # -------------------------------------------------------#
    map_out_path = save_path
    ...
    if map_mode == 0 or map_mode == 1:
        print("Load model.")
        yolo = YOLO(confidence=confidence, nms_iou=nms_iou, model_path=model_path,input_shape=input_shape)
...
if __name__ == "__main__":
    # opt = parser_opt()
    # for path in os.listdir(opt.model_path):
    #     if path.endswith('.pth'):
    #         model_path = os.path.join(opt.model_path, path)
    #         save_path = path[2:5] + '--' + path[-9:-4]
    #         save_path = os.path.join(opt.map_out_path, save_path)
    #         main(model_path, save_path)
    main('/root/autodl-tmp/yolov5-pytorch-main/logs/HiX/base_200/ep070-loss0.057-val_loss0.043.pth','output/HiXray/basel_70_640')