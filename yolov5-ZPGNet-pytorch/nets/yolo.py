...
from nets.net_blocks import ZPG, CM, SENet, NE, ZPG_MA_BN,ZPG_MA


# ---------------------------------------------------#
#   yolo_body
# ---------------------------------------------------#
class YoloBody_ZPG_MA(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, backbone='cspdarknet', pretrained=False, input_shape=[640, 640], MA_k=2):
        super(YoloBody_ZPG_MA, self).__init__()
        ...
        out_filters = [base_channels * 2, base_channels * 4, base_channels * 8, base_channels * 16]

        self.ZPG = ZPG()
        self.ZPG_MA = ZPG_MA(out_filters, MA_k=MA_k)
    ...
    def forward(self, x):
        ...
        feat3, feat2, feat1 = self.ZPG_MA([feat3, feat2, feat1, feat0], z)
...
class YoloBody_PM(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, backbone='cspdarknet', pretrained=False, input_shape=[640, 640],
        ...
        self.ZPG = ZPG()
        self.ZPG_MA_BN = ZPG_MA_BN(out_filters, MA_k=MA_k)
    ...
    def forward(self, x):
        ...
        feat3, feat2, feat1 = self.ZPG_MA_BN([feat3, feat2, feat1, feat0], z)
...
class YoloBody_base(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, backbone='cspdarknet', pretrained=False, input_shape=[640, 640]):
        super(YoloBody_base, self).__init__()
        depth_dict = {'s': 0.33, 'm': 0.67, 'l': 1.00, 'x': 1.33, }
        width_dict = {'s': 0.50, 'm': 0.75, 'l': 1.00, 'x': 1.25, }
        dep_mul, wid_mul = depth_dict[phi], width_dict[phi]
    ...
    def forward(self, x):
        #  backbone
        _, feat1, feat2, feat3 = self.backbone(x)
        if self.backbone_name != "cspdarknet":
            feat1 = self.conv_1x1_feat1(feat1)
            feat2 = self.conv_1x1_feat2(feat2)
            feat3 = self.conv_1x1_feat3(feat3)
