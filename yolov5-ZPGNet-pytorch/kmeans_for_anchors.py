...
def load_data(path, train_path):
    ...
    with open(train_path, 'r') as f:
        for p in f.readlines():
            p = p.strip()
            all_path.append(path + '/' + p + '.xml')

    for xml_file in tqdm(all_path):
...
if __name__ == '__main__':
    ...
    #   载入数据集，可以使用VOC的xml
    # -------------------------------------------------------------#
    path = '/root/autodl-tmp/yolo3-pytorch-master/VOCdevkit/HiXray/Annotations'
    train_path = '/root/autodl-tmp/yolo3-pytorch-master/VOCdevkit/HiXray/ImageSets/Main/train.txt'
    dataset = 'HiXray'
    # -------------------------------------------------------------#
    #   载入所有的xml
    ...
    # -------------------------------------------------------------#
    print('Load xmls.')
    data = load_data(path, train_path)
    ...
    plt.savefig("kmeans_for_anchors_%s_%s.jpg" % (dataset, str(input_shape[0])))
    plt.show()
    print('Save kmeans_for_anchors_%s_%s.jpg in root dir.' % (dataset, str(input_shape[0])))

    cluster = cluster[np.argsort(cluster[:, 0] * cluster[:, 1])]
    print('avg_ratio:{:.2f}'.format(avg_iou(data, cluster)))
    print(cluster)

    f = open("yolo_anchors_%s_%s.txt" % (dataset, str(input_shape[0])), 'w')
